...
public class FacturesQuery implements Query{

	List<Filter> sqlFilters;
	List<OrderBy> sqlOrderBys;
	
	protected FacturesQuery() {}
	
	public FacturesQuery(long recordid, QueryDefinition qd) {
		
		// MAPS THE NAME OF THE VAADIN FILTERS TO SQL FILTER
		DBFilterMapper fm = new DBFilterMapper(FacturesQueryMapping.sqlMapping);
		
		// MAP FILTERS
		qd.getFilters().add(new Compare.Equal("pmel_root", recordid));
		...
		for (int i = 0 ; i < qd.getSortPropertyIds().length ; i++) {
			sqlOrderBys.add(new OrderBy(
					(String) FacturesQueryMapping.sqlMapping.get((String) qd.getSortPropertyIds()[i]),
	...
	public Item constructItem() {
		return new BeanItem<BaseRsfA>(new BaseRsfA());
	}
	...
	public List<Item> loadItems(int startIndex, int count) {
		...
		try {
			...
			List<BaseRsfA> elements = na.getFactures(sqlFilters, sqlOrderBys, startIndex, count);

			// FILLS LIST OF ITEMS
			for (BaseRsfA element : elements) {
				items.add(new BeanItem<BaseRsfA>(element));
			}
	...
	public int size() {
		...
		try {
			// GETS THE LIST OF UPLOADED ELEMENTS
			size = na.getFacturesSize(sqlFilters);
		} catch (ActionException e) {
